          ...
          case T_STRING:
            switch ($state) {
              case T_CLASS:
                ...
                $si = $token[1];
                $classes[] = ['name' => $token[1], 'modifiers' => $mod, 'line' => $line, 'doc' => $doc];
                break;
              case T_FUNCTION:
                $state = self::STATE_FUNCTION_HEAD;
                $clsc = count($classes);
                if ($depth > 0 && $clsc) {
                    $classes[$clsc - 1]['functions'][$token[1]] = ['modifiers' => $mod, 'line' => $line, 'doc' => $doc];
                }
                break;
              case T_IMPLEMENTS:
              case T_EXTENDS:
                $clsc = count($classes);
                $classes[$clsc - 1][T_IMPLEMENTS == $state ? 'implements' : 'extends'][] = $token[1];
                break;
            }
            break;
        }
